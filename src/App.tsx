...
export default function App() {
  ...
  // Detect touch device
  useEffect(() => {
    const onTouch = () => { setHasTouch(true); window.removeEventListener('touchstart', onTouch) }
  ...
  useEffect(() => {
    ...
    } else if (resolvedTheme === 'dark' && prev === 'light' && color === '#000000') {
  ...
  const requestClear = useCallback(() => {
    if (settingsRef.current.confirmClear) {
      ...
      window.dispatchEvent(new CustomEvent('blackboard:query-stroke-count', { detail }))
      if (detail.count > 16) {
  ...
  const zoomIn = useCallback(() => {
    window.dispatchEvent(new CustomEvent('blackboard:zoom-step', { detail: 1.25 }))
  }, [])

  const zoomOut = useCallback(() => {
    window.dispatchEvent(new CustomEvent('blackboard:zoom-step', { detail: 0.8 }))
  }, [])

  const isDark = resolvedTheme === 'dark'

  const touchTools: { id: TouchTool; label: string; icon: JSX.Element }[] = [
    {
      id: 'draw', label: 'Draw',
      icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 14l3-1L13.5 4.5a1.4 1.4 0 0 0-2-2L3 11z" /><path d="M10.5 3.5l2 2" /></svg>,
    },
    {
      id: 'dashed', label: 'Dash',
      icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeDasharray="3 3"><line x1="2" y1="8" x2="14" y2="8" /></svg>,
    },
    {
      id: 'line', label: 'Line',
      icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><line x1="3" y1="13" x2="13" y2="3" /></svg>,
    },
    {
      id: 'erase', label: 'Erase',
      icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 14h8" /><rect x="2" y="5" width="12" height="7" rx="1" transform="rotate(-20 8 8.5)" /></svg>,
    },
  ]

  return (
    <>
      <Menu settings={settings} updateSettings={updateSettings} onExport={exportPng} onExportTransparent={exportTransparent} onClear={requestClear} zoom={zoom} onResetView={resetView} resolvedTheme={resolvedTheme} />
      <Canvas lineWidth={settings.lineWidth} lineColor={settings.lineColor} dashGap={settings.dashGap} showDotGrid={settings.showDotGrid} resolvedTheme={resolvedTheme} touchTool={touchTool} />
      {settings.showZoomControls && (
        <div className="fixed bottom-4 right-4 z-50 flex items-center gap-1.5">
          ...
          <span className={`text-xs tabular-nums w-10 text-center ${isDark ? 'text-white/50' : 'text-black/50'}`}>
            {Math.round(zoom * 100)}%
          </span>
          <button
            onClick={zoomIn}
      ...
      {hasTouch && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 p-1 rounded-lg border backdrop-blur-sm"
          style={{ background: isDark ? 'rgba(0,0,0,0.7)' : 'rgba(255,255,255,0.7)', borderColor: isDark ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.15)' }}
        >
          {touchTools.map(t => (
            <button
              key={t.id}
      ...
      {confirmingClear && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={() => setConfirmingClear(false)}>
          <div
          ...
          >
            <div className={`text-sm font-medium ${isDark ? 'text-white/90' : 'text-black/90'}`}>
              Clear all strokes?
            </div>
            <div className={`text-xs mt-2 ${isDark ? 'text-white/50' : 'text-black/50'}`}>
              Enter or {isMac ? '\u2318' : 'Ctrl'}+K to confirm &middot; Esc to cancel
            </div>
